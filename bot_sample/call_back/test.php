  ...
  if ($event instanceof \LINE\LINEBot\Event\MessageEvent) {
    if($event instanceof \LINE\LINEBot\Event\MessageEvent\TextMessage) {
      if($event->getText() === 'create') {
        $bot->replyMessage($event->getReplyToken(),new \LINE\LINEBot\MessageBuilder\TextMessageBuilder(createNewRichmenu(ACCESS_TOKEN)));
      }
        ...
        $bot->replyMessage($event->getReplyToken(),new \LINE\LINEBot\MessageBuilder\TextMessageBuilder(unlinkFromUser(ACCESS_TOKEN, $event->getUserId())));
      }
      else if($event->getText() === 'check') {
        $bot->replyMessage($event->getReplyToken(),new \LINE\LINEBot\MessageBuilder\TextMessageBuilder(checkRichmenuOfUser(ACCESS_TOKEN, $event->getUserId())));
      }
      else if(substr($event->getText(),0, 8) === 'upload::') {
        $bot->replyMessage($event->getReplyToken(),new \LINE\LINEBot\MessageBuilder\TextMessageBuilder(uploadRandomImageToRichmenu(ACCESS_TOKEN, substr($event->getText(), 8))));
      }
      else if(substr($event->getText(),0, 8) === 'delete::') {
        $bot->replyMessage($event->getReplyToken(),new \LINE\LINEBot\MessageBuilder\TextMessageBuilder(deleteRichmenu(ACCESS_TOKEN, substr($event->getText(), 8))));
      }
      else if(substr($event->getText(),0, 6) === 'link::') {
        $bot->replyMessage($event->getReplyToken(),new \LINE\LINEBot\MessageBuilder\TextMessageBuilder(linkToUser(ACCESS_TOKEN, $event->getUserId(), substr($event->getText(), 6))));
      }
      else {
